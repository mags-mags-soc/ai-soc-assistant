...
def build_email_html(alert: Alert, analysis: AIAnalysis) -> str:
    ...
    steps = "".join(f"<li>{s}</li>" for s in analysis.investigation_steps)
    return f"""\
<html><body style="font-family:Arial,sans-serif;color:#1a1a1a;">
  ...
  <table style="margin-top:12px;border-collapse:collapse;">
    ...
    <tr><td><b>Rule</b></td><td>{alert.rule.description} (level {alert.rule.level})</td></tr>
  ...
  <h3>Risk Assessment</h3><p>{analysis.risk_assessment}</p>
  <h3>Investigation Steps</h3><ol>{steps}</ol>
  <h3>MITRE Commentary</h3><p>{analysis.mitre_commentary}</p>
</body></html>"""
...
class EmailNotifier:
    ...
    def _build_message(self, alert: Alert, analysis: AIAnalysis) -> EmailMessage:
        msg = EmailMessage()
        msg["Subject"] = f"[SOC {analysis.risk_level.value.upper()}] {alert.rule.description}"
